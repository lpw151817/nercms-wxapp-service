...
public class AffairDao {
	 ...
	 * @return 事务列表
	 */
	public ArrayList<AffairModel> getSendAffairByStatus(String sponsorID,
			int status) {
		ArrayList<AffairModel> affairs = new ArrayList<AffairModel>();
		Cursor cursor = null;
		try {
			...
			cursor = db.query(DBConstants.AFFAIRS_TABLE_NAME,
					...
					"sponsor_id = ? and status = ?", new String[] { sponsorID,
							Integer.toString(status) }, null, null, null);
			while (cursor.moveToNext()) {
				affairs.add(createAffairFromCursor(cursor));
			}
		} finally {
	 ...
	 * @return 事务列表
	 */
	public ArrayList<AffairModel> getReceiveAffairByStatus(String sponsorID,
			int status) {
		ArrayList<AffairModel> affairs = new ArrayList<AffairModel>();
		Cursor cursor = null;
		try {
		...
		try {
			...
			if (type == 1) { // 查询发起任务
				cursor = db.rawQuery("SELECT COUNT(*) FROM "
						...
						+ " WHERE sponsor_id = ? and status = ?", new String[] {
			...
			if (type == 1) { // 查询发起任务
				cursor = db
						.rawQuery(
								"SELECT COUNT(*) FROM "
										+ DBConstants.AFFAIRS_TABLE_NAME
										+ " WHERE sponsor_id = ? and status = ? and is_read = 0",
								new String[] { userID, String.valueOf(status) });
			} else { // 查询接收任务
				cursor = db.rawQuery("SELECT COUNT(*) FROM "
						+ DBConstants.AFFAIRS_TABLE_NAME
	...
	public int getAffairIsReadByID(String affairID) {
		...
		try {
			...
			cursor = db.rawQuery("SELECT is_read FROM "
					+ DBConstants.AFFAIRS_TABLE_NAME + " WHERE affair_id = ? ",
					new String[] { affairID });
	...
	public ArrayList<AffairModel> getAffairByStatus(int status, int id) {
		...
		try {
			...
			cursor = db.query(DBConstants.AFFAIRS_TABLE_NAME,
					DBConstants.AFFAIR_ALL_COLUMNS, "status = ?",
					new String[] { Integer.toString(status) }, null, null,
	...
	public AffairModel getAffairById(String affairID) {
		...
		try {
			SQLiteDatabase db = dbHelper.getReadableDatabase();
			cursor = db.query(DBConstants.AFFAIRS_TABLE_NAME,
					DBConstants.AFFAIR_ALL_COLUMNS, "affair_id = ?",
	 ...
	 *            （由服务器端返回得到）
	 */
	public void updateAffairEndTime(String affairID, String endTime,
			String lastOperateTime) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put("end_time", endTime);
		values.put("is_read", 0); // 更新为未读
		values.put("status", DBConstants.AFFAIR_UNCOMPLETED); // 更新状态为进行中，针对延误任务的修改
	...
	private AffairModel createAffairFromCursor(Cursor cursor) {
		...
		return new AffairModel(affairID, type, sponsorID, title, description,
				beginTime, endTime, completeTime, lastOperateType,
				lastOperateTime, isRead, status);
	}
